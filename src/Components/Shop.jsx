...
const Shop = () => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            if (shop) {
                const foundwigs = shop.filter(wig => wig.gender === gender ); 
                if (foundwigs.length > 0) {
                    setNewWigs(foundwigs);
                } else {
                    console.log(`No wigs found for category: ${gender}`);
                }
            }
            setLoading(false);
        }, 1000);
    ...
    return (
        <div className='mt-16 px-6 md:px-14 lg:px-28'>
            <div className='text-center text-4xl font-medium pb-14 text-[#2A0134]'>
                <h1 className="uppercase">{gender}'s wig</h1>
            </div>
            ...
            <div className="mt-16 mx-0 md:mx-12 flex items-start justify-start gap-5">
                ...
                <p className='text-sm font-normal text-zinc-600 text-justify '>When you select a stylist hair, you are choosing a product that uses some of the most technologically advanced hair fibers. Many of our stylist hairs are created with heat-resistant fiber, allowing you to use a curling iron whenever you want. One of the best traits of stylist hair is that you rarely have to worry about styling it. We offer a fabulous selection of the best styles, so you can get on with your day with ease while looking absolutely amazing. Find exactly what you're searching for in our online catalog of the latest styles, whether it's short curly styles or long straight styles, with a wide range of color choices. You owe it to yourself to look and feel your best, and our stylist hair is sure to help you achieve that. Start shopping for your stylist hair today and fall in love with your soon-to-be new look.</p>
            </div>
        </div>
    );
};
